...
def value_iteration(mdp, gamma, epsilon):
    ...
    for state in mdp.get_states():
        value[state] = mdp.get_reward(state)
        prevStatesValues[state] = 0
        if mdp.is_terminal(state):
            prevStatesValues[state] = mdp.get_reward(state)
            tempValue[state] = mdp.get_reward(state)
    while any(abs(value[state]) - abs(prevStatesValues[state]) > epsilon for state in mdp.get_states()):
        iter += 1
        for state in mdp.get_states():
            prevStatesValues = value
            up = 0
            left = 0 
            right = 0
            down = 0
            ...
            for action in mdp.get_actions(state):
                probs = mdp.get_successor_probs(state, action)
                possibleStates = mdp.get_successor_probs(state, action).keys()
                for pState in possibleStates:
                    if action == "up":
                        up += probs[pState] * value[pState]
                    if action == "right":
                        right += probs[pState] * value[pState]
                    if action == "down":
                        down += probs[pState] * value[pState]
                    if action == "left":
                        left += probs[pState] * value[pState]
            tempValue[state] = mdp.get_reward(state) + gamma * max(up, right, left, down)
            #print(state, tempValue[state], mdp.get_reward(state), gamma, max(up, right, left, down), up, right, down, left )
        value = dict(tempValue)
    print(iter)
        #print(ascii_grid_policy(derive_policy(mdp,value )))
        #print(print(ascii_grid_utils(value)))
    return value
